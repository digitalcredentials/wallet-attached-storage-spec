...
* Perform an authenticated Create Space operation. The controller/invoker DID
  of the operation is set as the Space's `controller`

* If an `id` is provided in the Create Space request, it must be a `urn:uuid`.
...
Example error response (missing authorization, unable to determine controller):
